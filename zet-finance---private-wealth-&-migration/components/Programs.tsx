      ...
      <div className="container mx-auto px-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
           {services.map((service, idx) => (
             <div 
               key={idx} 
               className="group relative bg-slate-50 border border-slate-100 p-8 hover:bg-white hover:shadow-2xl hover:border-brand-gold/30 transition-all duration-500 cursor-pointer overflow-hidden rounded-sm flex flex-col items-center text-center h-[320px]"
               onClick={() => onQuoteClick(service.title)}
             >
                {/* Minimalist Card Layout */}
                <div className="mt-8 mb-6 inline-flex p-4 bg-brand-navy text-white group-hover:bg-brand-gold group-hover:text-brand-navy transition-colors duration-500 rounded-full shadow-lg">
                   {service.icon}
                </div>
                
                <h3 className="font-serif text-xl font-bold text-brand-navy mb-3 leading-tight px-2">{service.title}</h3>
                
                <p className="text-slate-500 text-sm leading-relaxed mb-auto px-2">
                   {service.subtitle}
                </p>
